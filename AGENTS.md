...
| `src/routes/index.tsx` | 首页改「分类分区」布局:按 lib/data 的 categories 上下分块(AI 学习在上、公众号文章在下,空分类不显示),卡片纯文字不放封面图;hero 统计卡点击滚动到分区;原类型筛选条/瀑布流已移除;2026-07-05 AI 分区内再按子分类分组(智能体/大模型/AI 工程…,未知归「其他」);2026-07-06「近期亮点」升级为「最近更新」模块(3 亮点卡 + 第 4–10 条时间列表),卡片日期补年份,7 天内资源加「新」徽标;2026-07-07 新增「工具」分区(数据驱动的 `TOOLS` 数组,链到 `public/` 下的独立静态页面如 `/projects/wechat-md/`,用 `<a>` 非 `<Link>`);2026-07-08 TOOLS 加「AI 每日简报」入口(`/ai-daily/`);2026-07-19 首页取数去掉 `limit:60`(时间倒序截断会把最老的公众号文章挤出首页,每日简报每天挤一篇;现取全量,近 1000 条时再做分页);2026-07-27 TOOLS 加「随读 SuiRead」外链卡片(排第一,href 用 `SUIREAD_APP_STORE_URL`),同时给 TOOLS 项加三个可选字段 `icon`/`cta`/`gaEvent`(有 icon 用 `<img>` 替掉扳手图标、cta 替掉「打开」、gaEvent 点击发 GA4;站内工具不传则表现不变);2026-08-05 TOOLS 加「Claude Code 学习站」卡(含 `hrefEn` 可选字段,en 界面链 `/en/claude-code/`);**2026-08-05 首页整理(用户嫌太长太乱,当时 21000px/183 张 AI 卡)**:每日自动内容(subcategory briefing/daily/claude-code)不再进首页分区(各有归档页+工具卡入口),每分区只平铺最新 6 张(`HOME_CARDS_PER_SECTION`),AI 子分类分组取消,分区尾部加「查看全部 N 篇」链到 `/resources?cat=<id>`;整理后首页约 5300px。合并冲突时保留本端分区布局 |
...
| `src/routes/resources.tsx`(SSR) | 2026-07-28 **加 `loader` 预取**:原先只有 `useQuery`,列表在客户端才渲染,服务端 HTML 正文仅 132 字符,而 AI 爬虫与部分搜索爬虫不执行 JS = 整个列表对它们不可见。加 loader 后 SSR 正文 11.7k 字符、105 个文章链接可见。**新增列表页务必带 loader**;2026-08-05 加 `validateSearch` 支持 `?cat=<分类id>` 直达筛选(首页「查看全部」用),组件用 strict:false 的 useSearch 读取以便 /en/resources 复用 |
...
| `public/claude-code/` + `.claude/commands/daily.md` + `CLAUDE-CODE-SITE.md` | 2026-08-05 新增「Claude Code 学习站」板块(线上 `/claude-code/`):系统课程(14 课线性)+ 实战 Tip(六簇可检索),**每天在本仓库跑 `/daily` 增量更新**(写一课 + 2 条 tip + 复核 + 部署,验证不过不 push)。状态接力文件 `public/claude-code/state.json`;页面唯一样式源 `assets/shared.css`(三档主题 跟随系统/浅色/深色);索引页靠 HTML 注释标记维护。配套改动:sync 脚本加 `claude-code/course`、`claude-code/tips` 扫描(subcategory `claude-code`)+ 标题剥「· Claude Code 学习站」尾缀,manifest skip 两个 template 页,`data.ts` AI 分类加 Claude Code 子分类,首页 TOOLS 加入口卡。详见 `CLAUDE-CODE-SITE.md`。合并冲突时保留本端 |
