...
### 阶段 4:承接转化 ✅ 2026-08-26 实现,本地实测通过,待部署与验收
- 内容:hero 新文案、关于页加厚、订阅入口统一、文章页文末"下一篇+订阅"。
- 验收标准:
  - [x] hero 不再是自我视角文案(阶段 1 已换成「每天讲透一个 AI 主题」,保鲜改造又加了实时数据条)
  - [x] 关于页含 我是谁/在产出什么/怎么找我 三段 —— 中英两版实测都成立
        (旧版是写死的五个「兴趣方向」,一年不动;英文页此前直接显示中文,本次一并 i18n)
        「在产出什么」用实时数据:三栏目各自条数与最新日期、手写长文数、工具数
  - [x] 文章文末有同栏目下一篇 + 订阅入口(前后篇导航原本就有,这次补上订阅卡)
- 订阅入口统一:新增 `components/SubscribeCard.tsx` 作为全站唯一入口,首页 / 文章文末 / 关于页共用。
  同时给两条路径 —— 留邮箱,或拿 `/rss.xml` 自己收。`SubscribeForm` 的中文硬编码文案也一并进了字典。

### 保鲜改造的一处自我修正:连更天数 → 近 30 天活跃天数
Hero 数据条最初写的是「连更 N 天」。2026-08-26 上线后实测露馅:8-25 空了一天,
连更就从 8 打回 1,而同期实际更新了 **22/30 天**。这个数字本来是要说明「这站还活着」,
漏一天就归零等于把话讲反。改成「N 天有更新(近 30 天)」—— 不会被单日空档抹掉,也没有夸大。
首页与关于页同步改。
